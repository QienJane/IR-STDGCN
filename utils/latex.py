...
def create_latex_table(results_folder, output_folder, methods, n_tasks, dataset):
    ...
    df.to_latex(os.path.join(output_folder, 'results.tex'),
                index=False,
                column_format='l' + 'c' * n_tasks,
                escape=False)

if __name__ == "__main__":
    # Save results in latex format
    results_folder = '/.../IR-STDGCN/output/'
    n_tasks = 7
    dataset = sys.argv[1]
    methods = sys.argv[2].split(' ')

    methods_str = sys.argv[2]


    latex_folder = os.path.join(results_folder, dataset, methods_str, 'LaTeX')
    print(f'latex_folder: {latex_folder}')
