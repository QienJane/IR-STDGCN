        ...
        for current_t_index in range(self.cfg.increm.max_task):
            ...
            if current_t_index == 1 and self.cfg.increm.load_best_checkpoint_train:
                ...
                if model_path is not None:
                    ...
                    raise FileNotFoundError(f"Model checkpoint not found in the log directory {self.args.log_dir}")
            ...
            dataset_module = importlib.import_module(f'.{self.args.dataset}',
                                                     package='datasets')
            DatasetClass = getattr(dataset_module, 'Dataset')

            self.train_dataset = DatasetClass(
                mode='train',
                split_type=self.args.split_type,
                cfg=self.cfg_data,
                cfg_xforms=self.cfg.transforms['train'],
                n_add_classes=self.add_classes,
                n_known_classes=self.known_classes,
                rm_global_scale=self.cfg.rm_global_scale,
                drop_seed=n_trial
            )
            self.val_dataset = DatasetClass(
                ...
                split_type=self.args.split_type,
                cfg=self.cfg_data,
                cfg_xforms=self.cfg.transforms['val'],
                n_add_classes=self.add_classes,
                n_known_classes=self.known_classes,
                rm_global_scale=self.cfg.rm_global_scale,
                drop_seed=n_trial
            ...
            if current_t_index == 0 and self.cfg.increm.load_pretrained_task0:
                ...
                pretrained_checkpoint_path = osp.join('/.../IR-STDGCN/models', self.args.dataset, self.cfg.model.name, f"trial_{n_trial+1}", 'checkpoints', 'model_best.pth.tar')
                if osp.exists(pretrained_checkpoint_path):
                    ...
                    raise FileNotFoundError(f"Pretrained checkpoint not found in {pretrained_checkpoint_path}")
            else:
                ...
                if self.gen_inverted_samples and (not self.is_distributed or self.args.gpu == 0):
                    inverted_sample_dir = osp.join(self.args.log_dir, 'inverted_samples')
                    os.makedirs(inverted_sample_dir, exist_ok=True)
                    self.generate_inverted_samples(
                        self.previous_teacher,
                        self.cfg.increm.learner.n_samples_per_class,
                        self.cfg.increm.learner.inversion_batch_size,
                        self.cfg.batch_size,
                        log_dir_task,
                        inverted_sample_dir,
                    )
            ...
            self.model.eval()
            # new teacher
            if self.cfg.increm.learner.type in ['deep_inversion', 'ir']:
            ...
            if self.cfg.increm.memory > 0:
                self.coreset_train = self.train_dataset.update_coreset(
                    self.coreset_train,
                    self.cfg.increm.memory,
        ...
        for current_t_index in range(cfg.increm.max_task):
            ...
            for test_mode in ['local', 'global', 'old', 'new']:
                ...
                if model_path is not None:
                    ...
                    raise FileNotFoundError(f"Model checkpoint not found in the log directory {log_dir_task}")
